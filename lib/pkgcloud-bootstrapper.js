...
var Bootstrapper = module.exports = function Bootstrapper(options) {
  ...
  // `options.compute` is a `pkgcloud.compute` client created by user.
  this.compute = options.compute;
  this.retryLimit = options.retryLimit || 3;
  this.remoteUser = options.remoteUser || 'root';
};
util.inherits(Bootstrapper, events2.EventEmitter2);

// ### function providerOptions
// Provides provider-specific options for bootstrapping servers.
Bootstrapper.prototype.providerOptions = {
  rackspace: function (options, keys) {
    return {
      personality: [
        {
  ...
  amazon: function (options, keys) {
    ...
    zone = options.zone || options.AvailabilityZone;
    if (zone) {
      result['Placement.AvailabilityZone'] = zone;
    }
  ...
  openstack: function (options, keys) {
    var keyname = options.keyname || options.key_name;
    if (keyname) {
...
Bootstrapper.prototype.createServer = function (options) {
  ...
  if (!options || !options.name || !options.image || !options.flavor) {
    throw new Error('`options.name`, `options.image` and `options.flavor` are required');
  }
  ...
  this.readKeys(function (err, keys) {
    ...
    // vs. `KeyName` (EC2).
    if (self.providerOptions[self.compute.provider]) {
      merge(options, self.providerOptions[self.compute.provider](options, keys));
    }

    creator = self._createServer(options);
...
Bootstrapper.prototype.runScriptRemotely = function (options) {
  ...
  var extra = options.commands || [];
  var emitter = new events.EventEmitter();
  var address = options.address || pkgcloud.compute.serverIp(options.server);
...
Bootstrapper.prototype._sshPoll = function (options, callback) {
  var self = this;
  var attempts = 0;
  var maxRetries = options.pollMax || 10;
